...
application/x-httpd-php route_map.php ( PHP script, ASCII text, with very long lines )
<?php
   
   function checkbox_format($crime_array = "") {
       // crime_array is a hack..
   ...
   include('session.php');
   $crime_types_list = "";
   checkbox_format($crime_types_list);
   ...
   if ($_POST["10-closest"]) { // use the k-closest
    ...
    //   echo "K set to ", strval($k);
       ...
       for($x = 0; $x < count($point_arr); $x++) {
           $lat = $point_arr[$x][2];
           $long = $point_arr[$x][3];
           $types = checkbox_format($crime_types_list);
...
<html">
   ...
   <div style="background-color:#b1bfca; color:#ffffff; display:flex; align-items:center; flex: 1">
      <h1 style="padding-left: 10px"><?php echo "VIEW ONLY: ".$route_name; ?></h1>
